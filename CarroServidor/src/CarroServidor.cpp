...
unsigned long lastTimeDistancia = 0;
unsigned long distanciaDelay = 50;
unsigned long lastTimeDistanciaR2 = 0;
unsigned long distanciaDelayR2 = 50;
unsigned long lastTimeDistanciaPOS = 0;
unsigned long distanciaDelayPOS = 40;
unsigned long lastTimeDistanciaPOSR2 = 0;
unsigned long distanciaDelayPOSR2 = 40;
unsigned long lastTimeDistanciaR1 = 0;
unsigned long distanciaDelayR1 = 50;
int conectar = 0;
//----------------------------------------------------------------------------------------
long randomGiro;
long randomIzqDer;
...
//#define radio_chasis 6//5.95 Robot original placa sin pines sobresalientes
#define radio_chasis 6//5.85
float res;
...
const double Const = 0.048245, distRuedas = 0.119;
...
//----------------------------------------------------------------------------------------
uint8_t broadcastAddress[] = {0x7C, 0x9E, 0xBD, 0x60, 0xF4, 0x84};

//uint8_t broadcastAddress[] = {0x24, 0x6F, 0x28, 0x24, 0xD0, 0x20};
//uint8_t broadcastAddress[] = {0x7C, 0x9E, 0xBD, 0x60, 0xF4, 0x84};// MAC Esclavo
//7C:9E:BD:60:F4:84
//24:6F:28:24:D9:44 --> Placa nueva
String varR1;
String Exito = "Éxito";
String Error = "Error";
String Vacio = " ";
...
String incomingPlaca; // Mensaje de la placa 2
String incomingColorFront, incomingColorTras, incomingEstado ,incomingEncontrado, incomingEncontrado3;
float incomingSenDist;
int incomingconect, incomingR, ContNOW ,contador; // Valor de i
String success;
int front, tras;
typedef struct struct_message {
    ...
    float SenDist;
    int conect, R, L;
    String ColorFront, ColorTras, Estado, EO, EO3;
} struct_message;
...
String frontalCuatroR1[16] = {}, trasCuatroR1[16] = {};
String frontalCuatroR2[16] = {}, trasCuatroR2[16] = {}; // Para guardar datos de R2
int count, sum,f, j, i;
//----------------------------------------------------------------------------------------
...
void OnDataRecv(const uint8_t * mac, const uint8_t *incomingData, int len) {
  ...
  Serial.println(len);
  incomingPlaca = incomingReadings.Placa;
  incomingSenDist = incomingReadings.SenDist;
  incomingconect = incomingReadings.conect; // contador2 de robot 2
  incomingR = incomingReadings.R; // j2 de robot 2
  incomingColorFront = incomingReadings.ColorFront;
  incomingColorTras = incomingReadings.ColorTras;
  incomingEstado = incomingReadings.Estado;
  incomingEncontrado = incomingReadings.EO;
  incomingEncontrado3 = incomingReadings.EO3;
}
void Datos(){
  strcpy(myData.Placa , "Placa Emisora 1");
  myData.SenDist = distanciaMili;//15.32;
  myData.conect = conectar;
  myData.R = contador;//2;//contador
  myData.ColorFront = Color_front;//"Blanco";
  myData.ColorTras = Color_tras;//"Negro";
  myData.Estado = success;
  myData.EO = Encontrado;

  esp_err_t result = esp_now_send(broadcastAddress, (uint8_t *) &myData, sizeof(myData));
   
  if (result == ESP_OK) {
    varR1 = Exito;//"Éxito en la entrega desde R1";
  }
  else {
    varR1 = Error;//"Error en la entrega desde R1";
  }
  delay(60); //60
}
//----------------------------------------------------------------------------------------
void SETUP_SENSOR_DISTANCIA(){
  ...
  if (!sensor.init())
  {
    Serial.println("¡Falla en la inicializacion del sensor!");
    while (1) {}
  }
...
void CalibracionInalambrica(){ 
  ...
  //int canalesFront[8] = {canal_0,canal_1,canal_2,canal_3,canal_4,canal_5,canal_6,canal_7};
  //int canalesTrase[8] = {canal_8,canal_9,canal_10,canal_11,canal_12,canal_13,canal_14,canal_15};
  //for(byte i = 0; i <=15 ; i++)
    //{
...
void Colores(int PWMBlancoMax_F,int PWMBlancoMax_T, int PWMNegroMin_F, int PWMNegroMin_T, int PWMGrisMin_F, int PWMGrisMax_F,int PWMGrisMin_T, int PWMGrisMax_T)
{
    ...
    //Trasero ------------------------------------------------------------------------
    // if(lectura_color_trase > PWMGrisMin_F){ // lectura_color_trase  PWMGrisMin_T
    //   if(lectura_color_trase < PWMGrisMax_T){ // lectura_color_trase PWMGrisMax_T
    //    Color_tras = Gris;
    //   }
    // } 
...
//----------------------------------------------------------------------------------------
// Capturamos los datos para mostrar en el servidor web
String getCalibracion() // ROBOT 2
  {  // Datos del robot 2 mostrados en el servidor web:
    readings["BlancoMaxF"] = String(PWMBlancoMaxG_F);
    readings["BlancoMaxT"] = String(PWMBlancoMaxG_T); //PWMBlancoMaxG_T
    readings["NegroMinF"] = String(PWMNegroMinG_F);
    readings["NegroMinT"] = String(PWMNegroMinG_T);   //PWMNegroMinG_T
    readings["GrisMinF"] = String(PWMGrisMinG_F);  
    readings["GrisMinT"] = String(PWMGrisMinG_T); //PWMGrisMinG_T
    readings["GrisMaxF"] = String(PWMGrisMaxG_F);
    readings["GrisMaxT"] = String(PWMGrisMaxG_T); //PWMGrisMaxG_T
    String EstadoCalibracion = JSON.stringify(readings);
    return EstadoCalibracion;
  }
String getEstadoR1() // ROBOT 2
  {  // Datos del robot 2 mostrados en el servidor web:
    readings["Dato_char"] = String(incomingPlaca); //incomingR ---> contador R2   incomingPlaca
    readings["Dato_int"] = String(incomingSenDist); // incomingconect ---> ContNOW  incomingSenDist
    //readings["Dato_float"] = String(incomingconect);
    //readings["Dato_bool"] = String(incomingR); // Dato mapa    
    readings["ColorFront"] = String(incomingColorFront); //incomingColorFront
    readings["ColorTras"] = String(incomingColorTras);//incomingColorTras  readings["Color"] = String(incomingEncontrado);    
    readings["var"] = String(incomingEstado); //varR1
    readings["Encontrado"] = String(incomingEncontrado); //varR1
    readings["Encontrado2"] = String(incomingEncontrado3); //varR1 
    String EstadoString = JSON.stringify(readings);
    return EstadoString;
  }
String getEstadoR2() // ROBOT 1
  {  
    readings["Dato_charR2"] = String(myData.Placa); // myData.R -->contador R1myData.Placa
    readings["Dato_intR2"] = String(myData.SenDist); // ContNOW  myData.SenDist    
    //readings["Dato_floatR2"] = String(myData.conect);
    //readings["Dato_boolR2"] = String(myData.R); // contador    
    readings["ColorFrontR2"] = String(myData.ColorFront); //Color_Front   myData.ColorFront
    readings["ColorTrasR2"] = String(myData.ColorTras); // myData.ColorTras   Color_Tras-- readings["ColorR2"] = String(myData.EO);    
    readings["varR2"] = String(myData.Estado);//success
    readings["EncontradoR2"] = String(Encontrado); //varR1
    String EstadoR2String = JSON.stringify(readings);
    return EstadoR2String;
  }   
String getEstadoPOS3x3() // ROBOT 1. --- FUNCIONA BIEN---
  { /*Hacer otra condición para cuando es de 4x4. Enviar desde el otro robot (R2) otro valor para
  saber cuando es de 3x3 y 4x4. Ingresarlos en las funciones de 3x3 y 4x4
  */
    // frontalTres[incomingR] = incomingColorFront;
    // trasTres[incomingR] = incomingColorTras;
    frontalTres[f] = Color_front;
    //delay(900);
    trasTres[f] = Color_tras; //Color_tras
    readings["a1f"] = String(frontalTres[1]);//String(myData.delantero[0]);
    readings["a1t"] = String(trasTres[1]);//String(myData.trasero[0]);
    readings["a2f"] = String(frontalTres[2]);
    readings["a2t"] = String(trasTres[2]);
    readings["a3f"] = String(frontalTres[3]);//String(myData.delantero[2]);
    readings["a3t"] = String(trasTres[3]);//String(myData.trasero[2]);
    readings["a4f"] = String(Vacio);//String(myData.delantero[2]);
    readings["a4t"] = String(Vacio);
    readings["b4f"] = String(Vacio);
    readings["b4t"] = String(Vacio);   
    readings["b3f"] = String(frontalTres[4]);
    readings["b3t"] = String(trasTres[4]);
    readings["b2f"] = String(frontalTres[5]);
    readings["b2t"] = String(trasTres[5]); 
    readings["b1f"] = String(frontalTres[6]);
    readings["b1t"] = String(trasTres[6]);
    readings["c1f"] = String(frontalTres[7]);
    readings["c1t"] = String(trasTres[7]);
    readings["c2f"] = String(frontalTres[8]);
    readings["c2t"] = String(trasTres[8]);
    readings["c3f"] = String(frontalTres[9]);
    readings["c3t"] = String(trasTres[9]);  //Vacio
    readings["c4f"] = String(Vacio);
    readings["c4t"] = String(Vacio);
    readings["d1f"] = String(Vacio);
    readings["d1t"] = String(Vacio);
    readings["d2f"] = String(Vacio);
    readings["d2t"] = String(Vacio);
    readings["d3f"] = String(Vacio);
    readings["d3t"] = String(Vacio);
    readings["d4f"] = String(Vacio);
    readings["d4t"] = String(Vacio);    
    String EstadoPOS3x3 = JSON.stringify(readings);
    return EstadoPOS3x3;
  }
String getEstadoRobotUno(){ // Carro 1
    frontalCuatroR1[j] = Color_front;
    trasCuatroR1[j] = Color_tras;   
    frontalCuatroR2[incomingR] = incomingColorFront;
    trasCuatroR2[incomingR] = incomingColorTras;

    readings["a1f"] = String(frontalCuatroR1[0]);//String(myData.delantero[0]);
    readings["a1t"] = String(trasCuatroR1[0]);//String(myData.trasero[0]);
    readings["a2f"] = String(frontalCuatroR1[1]); /// desaparece ?
    readings["a2t"] = String(trasCuatroR1[1]); /// desaparece ?
    readings["a3f"] = String(frontalCuatroR1[2]);//String(myData.delantero[2]); /// desaparece ?
    readings["a3t"] = String(trasCuatroR1[2]);//String(myData.trasero[2]); /// desaparece ?
    readings["a4f"] = String(frontalCuatroR1[3]);//String(myData.delantero[2]);
    readings["a4t"] = String(trasCuatroR1[3]);
    readings["b4f"] = String(frontalCuatroR1[4]);
    readings["b4t"] = String(trasCuatroR1[4]);    
    readings["b3f"] = String(frontalCuatroR1[5]);//
    readings["b3t"] = String(trasCuatroR1[5]);
    readings["b2f"] = String(frontalCuatroR1[6]);//
    readings["b2t"] = String(trasCuatroR1[6]); 
    readings["b1f"] = String(frontalCuatroR1[7]);
    readings["b1t"] = String(trasCuatroR1[7]);

    
      readings["d4f"] = String(frontalCuatroR2[0]);
      readings["d4t"] = String(trasCuatroR2[0]);
    
      readings["d3f"] = String(frontalCuatroR2[1]);
      readings["d3t"] = String(trasCuatroR2[1]);
    
      readings["d2f"] = String(frontalCuatroR2[2]);
      readings["d2t"] = String(trasCuatroR2[2]);
    
      readings["d1f"] = String(frontalCuatroR2[3]);
      readings["d1t"] = String(trasCuatroR2[3]);
    
      readings["c1f"] = String(frontalCuatroR2[4]);//
      readings["c1t"] = String(trasCuatroR2[4]);  
    
      readings["c2f"] = String(frontalCuatroR2[5]);//
      readings["c2t"] = String(trasCuatroR2[5]);
    
      readings["c3f"] = String(frontalCuatroR2[6]);//
      readings["c3t"] = String(trasCuatroR2[6]);
    
      readings["c4f"] = String(frontalCuatroR2[7]);
      readings["c4t"] = String(trasCuatroR2[7]);
      
    // readings["c1f"] = String(frontalCuatroR1[9]);
    // readings["c1t"] = String(trasCuatroR1[9]);
    // readings["c2f"] = String(frontalCuatroR1[10]);
    // readings["c2t"] = String(trasCuatroR1[10]);
    // readings["c3f"] = String(frontalCuatroR1[11]);
    // readings["c3t"] = String(trasCuatroR1[11]);
    // readings["c4f"] = String(frontalCuatroR1[12]);
    // readings["c4t"] = String(trasCuatroR1[12]);
    // readings["d1f"] = String(frontalCuatroR1[13]);
    // readings["d1t"] = String(trasCuatroR1[13]);
    // readings["d2f"] = String(frontalCuatroR1[14]);
    // readings["d2t"] = String(trasCuatroR1[14]);
    // readings["d3f"] = String(frontalCuatroR1[15]);
    // readings["d3t"] = String(trasCuatroR1[15]);
    // readings["d4f"] = String(frontalCuatroR1[16]);
    // readings["d4t"] = String(trasCuatroR1[16]);
    String EstadoPOS4x4R1 = JSON.stringify(readings);
    return EstadoPOS4x4R1;
  }

/*String getEstadoRobotDos(){ // Carro 2
    frontalCuatroR2[incomingR] = incomingColorFront;
    trasCuatroR2[incomingR] = incomingColorTras;
    readings["d4f"] = String(frontalCuatroR2[0]);
    readings["d4t"] = String(trasCuatroR2[0]);
    ...
    readings["c4f"] = String(frontalCuatroR2[7]);
    readings["c4t"] = String(trasCuatroR2[7]);
    readings["b4f"] = String(frontalCuatroR2[8]);
    readings["b4t"] = String(trasCuatroR2[8]);    
    readings["b3f"] = String(frontalCuatroR2[9]);
    readings["b3t"] = String(trasCuatroR2[9]);
    readings["b2f"] = String(frontalCuatroR2[10]);
    readings["b2t"] = String(trasCuatroR2[10]); 
    readings["b1f"] = String(frontalCuatroR2[11]);
    readings["b1t"] = String(trasCuatroR2[11]);
    readings["a1f"] = String(frontalCuatroR2[12]);//String(myData.delantero[0]);
    readings["a1t"] = String(trasCuatroR2[12]);//String(myData.trasero[0]);
    readings["a2f"] = String(frontalCuatroR2[13]);
    readings["a2t"] = String(trasCuatroR2[13]);
    readings["a3f"] = String(frontalCuatroR2[14]);//String(myData.delantero[2]);
    readings["a3t"] = String(trasCuatroR2[14]);//String(myData.trasero[2]);
    readings["a4f"] = String(frontalCuatroR2[15]);//String(myData.delantero[2]);
    readings["a4t"] = String(trasCuatroR2[15]);
    
    String EstadoPOS4x4R2 = JSON.stringify(readings);
    return EstadoPOS4x4R2; 
  } */
...
void Avance2(float DistRequerida, int Escena){
  ...
  else if (Escena == 4) // Comunicación 4x4
  {    
    j = j+1;
...
void Avance2giro(int DireccionGiro, int Escena/*, int DistMayoroMenor*/){
  if (Escena == 3) // Comunicación 3x3
  {
    ...
    /*if(DistMayoroMenor == 1){
      while (Dist_rec < 9.34625 && distanciaMili > 120)
      {
      giro(DireccionGiro); // Salida Dist_rec
      Colores(PWMBlancoMaxG, PWMNegroMinG, PWMGrisMinG, PWMGrisMaxG); 
      SENSOR_DISTANCIA();         
      Datos();
      EnvioDatos();
      } 
    }
    else if (DistMayoroMenor == 2)
    {
      while (Dist_rec < 9.34625 && distanciaMili < 120)
      {
      giro(DireccionGiro); // Salida Dist_rec
  ...
  {    
    ...
    RizqAux.write(0);
    RderAux.write(0);
    while (Dist_recAux < datoGiro - 35) // carro 1 co -40
      {
      giro(DireccionGiro); // Salida Dist_rec
      Colores(PWMBlancoMaxG_F, PWMBlancoMaxG_T, PWMNegroMinG_F, PWMNegroMinG_T,PWMGrisMinG_F, PWMGrisMaxG_F, PWMGrisMinG_T, PWMGrisMinG_T); 
      SENSOR_DISTANCIA();         
...
// CUANDO EL CARRO 1 TERMINA LOS 27 CM, NO ENVIA LOS DATOS AL INSTANTE DEL CARRO 2
//EnvioDatos();
//EnvioDatos3x3();
  if (arena == 3) // Un Robot
  {
    count = 9;
    for (i = 0; i < count; i++) // para 3x3 count == 8 (9)
    {
      //EnvioDatos3x3();
      EnvioDatos();
      delay(500);
      if ( (Dist <= 27600*i) && (Dist >= 27000*i))
      {
        ...
        while (i == 8 && count == 9)
        {
          Encontrado = "Mapeo Completo!";
          EnvioDatos();
          EnvioDatos3x3();//EnvioDatosPos3x3();
          motor1.freno(); motor2.freno();
        }       
      }
        if (i == 2 || i == 3)
      ...
      EnvioDatos();
      Datos(); 
      Avance2(27000, 3); // Output: Dist = Dist + Dist_recAux
      delay(500);   
    }
    //----------------------------------------------------------------------------------------
    //Solucionar los tiempos de envio de datos.
    EnvioDatos3x3();//EnvioDatosPos3x3();//EnvioDatos(); ????????
    EnvioDatos();
  ...
  else if (arena == 4) // Dos robot's
  {
    ...
    {
      //j = j+1; // i=8, j sera 9 "j" Aumentara en avance 2
      //EnvioDatos4x4();
      EnvioDatos();
      delay(1500);
      if ( (Dist <= 27600*i) && (Dist >= 27000*i))
      {
        ...
        Colores(PWMBlancoMaxG_F, PWMBlancoMaxG_T, PWMNegroMinG_F, PWMNegroMinG_T,PWMGrisMinG_F, PWMGrisMaxG_F, PWMGrisMinG_T, PWMGrisMinG_T);
        contador = contador + 1;
        ContNOW = contador + incomingconect;        
        Datos();
        EnvioDatos();
        EnvioDatos4x4();EnvioDatos4x4();
        delay(1000); 
        while (ContNOW == 15) // Es 15 porque el R1 envia el dato inmediatamente a R2 pero R2 no envia inmediatamente un "1" a R1.
        {
          Encontrado = "Mapeo Completo!";
          incomingEncontrado = "Mapeo";
          incomingEncontrado3 = "Completo!";
          EnvioDatos();
          EnvioDatos4x4();
          motor1.freno(); motor2.freno();
        }      
      }      
        while (i == 15 && count == 16) // i == 15
        {
          EnvioDatos();
          //EnvioDatos4x4();
          motor1.freno(); motor2.freno();
      ...
      //EnvioDatos4x4();EnvioDatos4x4();//EnvioDatosPos3x3();//EnvioDatos();
      EnvioDatos();
      Datos(); 
      delay(500);         
      Avance2(27000, 4); // Output: Dist = Dist + Dist_recAux
      //EnvioDatos4x4();
      delay(500);   
    }
  EnvioDatos4x4();//EnvioDatos4x4();
...
//----------------------------------------------------------------------------------------
//Habilitar rutina de calibración 
   server.on("/Blanco", HTTP_GET, [](AsyncWebServerRequest *request){
     conectar = 1;    
     //reset();    
    /*En negro será el menor numero tomado
     en gris se respetan los valores de min y max. */
     request->send(50, "text/plain", "OK");
   });
  server.on("/Gris", HTTP_GET, [](AsyncWebServerRequest *request){
    conectar = 2;    
    //reset();  
    request->send(50, "text/plain", "OK");
  });
  server.on("/Negro", HTTP_GET, [](AsyncWebServerRequest *request){
    conectar = 3;    
    //reset();  
    request->send(50, "text/plain", "OK");
  });
  server.on("/ResetCalibracion", HTTP_GET, [](AsyncWebServerRequest *request){
    conectar = 4;
    request->send(50, "text/plain", "OK");
  });
  server.on("/BorrarB", HTTP_GET, [](AsyncWebServerRequest *request){
    conectar = 5;
    request->send(50, "text/plain", "OK");
  });
  server.on("/BorrarG", HTTP_GET, [](AsyncWebServerRequest *request){
    conectar = 6;
    request->send(50, "text/plain", "OK");
  });
  server.on("/BorrarN", HTTP_GET, [](AsyncWebServerRequest *request){
    conectar=7;    
    request->send(50, "text/plain", "OK");
  });
  ...
  // Realizar edición de esta parte para poder obtener el dato de posición inicial
  server.on("/Escena1", HTTP_GET, [](AsyncWebServerRequest *request){
    //contador = 0;
    dist_m = dist_m + Dist_rec;
    Dist_rec = 0;
    conectar = 10;
  ...
  server.on("/Escena2", HTTP_GET, [](AsyncWebServerRequest *request){
    conectar = 9;
    request->send(50, "text/plain", "OK");
  });
  server.on("/Escena3", HTTP_GET, [](AsyncWebServerRequest *request){
    conectar = 11;
    request->send(50, "text/plain", "OK");
  });
  server.on("/Escena4", HTTP_GET, [](AsyncWebServerRequest *request){
    conectar = 12;
    request->send(50, "text/plain", "OK");
  });
//----------------------------------------------------------------------------------------
  ...
  events.onConnect([](AsyncEventSourceClient *client){
    if(client->lastId()){
      Serial.printf("Cliente reconectado! El último mensaje ID que recibió es: %u\n", client->lastId());
    }
...
void loop() {
    ...
    EnvioDatos();
    Serial.println(i);
    Serial.println(incomingPlaca);
    Serial.println(incomingSenDist);
    Serial.println(incomingconect);
    Serial.println(incomingR);
    Serial.println(incomingColorFront);
    Serial.println(incomingColorTras);
    Serial.println(incomingEstado);
    Serial.println(contador);
    Serial.println(ContNOW);
    delay(50);
  if(conectar == 1){
    //Datos();
    RizqAux.write(0);
    ...
    while (Dist_recAux < datoCal) // res = 37.38495258 (-500 carro 2)
    {
      ...
      //PWMBlancoMinG = Min_Calibracion;
      //PWMBlancoMaxG = Max_Calibracion + 10;   
    ...
    while (Dist_recAux < datoCal) //ProPulsos = 7748 -40 = 7708 pulsos
    {
      ...
      //PWMGrisMinG = Min_Calibracion;
      //PWMGrisMaxG = Max_Calibracion + 10;
      giro(3);
      Datos();Datos(); 
      EnvioDatos();
    }
    ...
    while (Dist_recAux < datoCal)
    {
      ...
      //PWMNegroMinG = Min_Calibracion - 10;
      //PWMNegroMaxG = Max_Calibracion;
      giro(3);
      Datos();Datos();
      EnvioDatos();
    }
  ...
  else if(conectar == 4){ //Reset calibración
  ...
  else if(conectar == 5){ // Borrar blanco
  ...
  else if(conectar == 6){ // Borrar gris --------------------// Guardamos los valores y hacemos el seguimiento de caminos:
  ...
  else if(conectar == 7){ //Borrar negro ---------------------// A prubea
    /*nuevaDer = Rder.read();
    nuevaIzq = Rizq.read();
  ...
  else if(conectar == 8)
  {
  ...
  else if(conectar == 9){  //Escenario 1: Un agente busca el blanco
    SENSOR_DISTANCIA();
    Datos();
    EnvioDatos();
    //void Colores(int PWMBlancoMax_F,int PWMBlancoMax_T, int PWMNegroMin_F, int PWMNegroMin_T, int PWMGrisMin_F, int PWMGrisMax_F,int PWMGrisMin_T, int PWMGrisMax_T)
  ...
  else if(conectar == 10){ // Escenario 2: Comienza en A1 y mapea
    // Agregar sensor de distancia a 80 mm según el dato mostrado en el servidor web
    Datos(); // Envio los datos al robot 2
    Escenario(3);    
  }
  else if (conectar == 11){ //Escenario 3: Mapeo 4x4 con dos robot´s
    Datos(); //Envio los datos al robot 2
    Escenario(4);
  }
  else if (conectar == 12){ //Escenario 4: Busqueda de un blanco con dos agentes.
    ...
    - Ir guardando una variable en una matriz hasta completar las 9 o 16 posiciones.
    - enviar un dato cuando cada uno encuentre un blanco.
    */
   SENSOR_DISTANCIA();
    Datos();
    EnvioDatos();
    ...
    while(Color_front == Blanco && Color_tras == Blanco){
      //motor1.freno();motor2.freno();
      // Blanco encontrado !!! ¿Enviar mensaje?
      Encontrado = "Blanco encontrado";
      SENSOR_DISTANCIA();
      Datos();
      EnvioDatos();
